...
function App() {
  ...
  const [updateModalIsOpen, setUpdateModelIsOpen] = useState(false);
  ...
  // 수정
  function handleUpdate(up_id) {
    console.log("App.handleUpdate", up_id);
    setUpdateModelIsOpen(updateModalIsOpen => !updateModalIsOpen); // 논리값 처리 New 방법
    // setUser(up_id); // 수정할 id 저장
  }
  // function handleUpdateConfirm(){
  //   console.log("App.handleUpdateConfirm");
  //   setModelIsOpen(updateModalIsOpen => !updateModalIsOpen);
  // }

  // function handleUpdateSubmit(updateInputs){
  //   let new_userList = userList.map(function(v, i, arr){
  //     if(arr[i].id = updateInputs.id){
  //       return arr[i] = updateInputs;
  //     }
  //   });
  //   setUserList(new_userList);
  ...
  function handleUpdateConfirm() {
    console.log("App.handleUpdateConfirm");
    setUpdateModelIsOpen(updateModalIsOpen => !updateModalIsOpen);

    var new_userList = userList.map((row, idx) => {if(row.id == user){user = inputs}}); // 배열에서 어떤 값을 삭제할 때. 삭제할 값을 뺀 나머지를 리턴
    console.log(new_userList);
    setUserList(new_userList);
  }
  ...
  return (
    <div className="App">
      ...
      <dialog open={updateModalIsOpen}>
        <h2>진짜로 수정할까요?</h2>
        <form onSubmit={handleUpdateSubmit}>
                id <input type="text" name="id" onChange={handleChange} value={inputs.id} disabled /><br />
        ...
        <button onClick={() => setModelIsOpen(updateModalIsOpen => !updateModalIsOpen)}>cancel</button>
